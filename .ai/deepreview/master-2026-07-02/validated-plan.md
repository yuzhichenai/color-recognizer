# Validated Implementation Plan — master — 2026-07-02
...
**Validation results: 8 approved, 0 revised, 0 rejected.**
...
**Priority:** critical
**Validation:** approved
**Validation notes:** Verified `selectColor()` at line 397-404; `playPickSound()` is called unconditionally. All callers confirmed: `onCanvasClick`/`onMouseUp` use default `saveToHist=true` (sound plays), `onMouseMove` eyedropper path passes `false` (sound suppressed). No callers broken by the gate.
...
**Priority:** critical
**Validation:** approved
**Validation notes:** Verified `sampleSize` is declared at line 36 and set by grid buttons at line 139. `getRegionAvg()` (line 395) handles coordinate clamping internally. Edge cases: `sampleSize=1` → 1×1 region (single pixel), correct. Replaced guard simplifies the redundant `mode==='eyedropper'||mode!=='click'` to just `mode!=='click'`, also addressing Fix 6.
...
**Priority:** warning
**Validation:** approved
**Validation notes:** Confirmed the DOM nesting: `<div class="hsl-sliders">` at line 110 has no explicit closing tag before `copyBtn` at line 123, so browsers nest copyBtn inside hsl-sliders. The inserted `</div>` at 14-space indentation matches hsl-sliders' opening indent (line 110). After the fix, `copyBtn` becomes a sibling of hsl-sliders inside color-details. The line 124 `</div>` then correctly closes color-details.
...
**Priority:** warning
**Validation:** approved
**Validation notes:** Grep confirmed no other references to `_locked`, `saveLocks`, `loadLocks`, or `isLocked` in source files (AGENTS.md/CHANGELOG.md references are documentation only). `_locked` is never populated (no UI ever calls `.add()`), and the delete handler at line 438 doesn't check `isLocked()`, so the lock is never enforced. Removal has zero functional impact.
...
**Priority:** warning
**Validation:** approved
**Validation notes:** Verified `sharePalette()` at line 486 generates URLs as `?p=hex1,hex2,...` (no `#` prefix). Fix 5 parses this correctly. `loadHist()` at line 165 runs after this code and reads from localStorage which was already written by `addHist`/`saveHist()`, so the data is consistent. Condition `_history.length === 0` prevents double-loading when localStorage already has history (though `loadHist()` hasn't been called yet at this point in init). Fix 4 must be applied first so that `_locked = loadLocks()` is removed from line 114 before Fix 5 inserts.
...
**Priority:** suggestion
**Validation:** approved
**Validation notes:** The parse-time `$$('.ctx-item').forEach(...)` at line 516 works correctly because the script loads at end of `<body>`, but is architecturally inconsistent with all other event bindings in `init()`. Moving it after line 132 (canvas event bindings) is safe: `init()` runs on `DOMContentLoaded` (line 535), so DOM is ready. `init()` is only called once, so no duplicate listener risk.
**Approach:** Move the `$$('.ctx-item').forEach(...)` binding from script-parse time into `init()` for architectural consistency with all other event bindings. Place it after the DOM-reliant event setup (after line 132).
**Code change:**
```javascript
...
**Priority:** suggestion
**Validation:** approved
**Validation notes:** Verified `onKeyDown` handles `0` key (line 498, reset zoom) and `S` key (line 502, extract dominant). Help modal at lines 276 and 280 already lists these shortcuts. Placeholder text `0 100%` is a compact form of "重置缩放 100%" — acceptable for the limited space.
...
## Order of Operations
- **Fix 4 (dead lock code) must be applied before Fix 5 (share palette).** Fix 4 removes `_locked = loadLocks();` from line 114, and Fix 5 inserts code immediately after that same line.
...
- Line numbers in the plan are approximate and will shift as preceding fixes modify the files. Match by content, not by line number.

## Risk
- **Fix 1 (pick sound):** Zero risk. The gate `if (saveToHist)` only suppresses sound when `selectColor(color, false)` is called (eyedropper mousemove path). All intentional picks use the default `saveToHist=true`.
- **Fix 2 (sampleSize):** The `getRegionAvg` call uses `getRegionAvg(ic.x-half, ic.y-half, ic.x+half, ic.y+half, s)` which already clamps coordinates internally. No risk of out-of-bounds access. Edge-case: `sampleSize` of 1 produces a 1×1 region (equivalent to single pixel), which is correct.
- **Fix 3 (missing `</div>`):** Browser-recovered gracefully before; fixing the structure has no behavioral impact but corrects the DOM for any future CSS targeting of child selectors.
...
- **Fix 5 (share URL):** The URL parsing only activates when `_history.length === 0` at init time, preventing double-loading if localStorage already has history. `addHist` → `saveHist()` writes to localStorage before `loadHist()` at line 165 reads it back, so data is consistent. Minor inefficiency: `addHist` calls `renderHist()` per item; acceptable for HIST_MAX=20.
- **Fix 7 (context menu):** No behavioral change. `init()` runs once on `DOMContentLoaded`, so no duplicate listener risk.
- **Fix 8 (shortcuts):** Purely a UI text change — no functional impact.
