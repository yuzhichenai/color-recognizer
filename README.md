...
> 上传图片，悬停/点击/框选取色，手动输入 HEX，一键复制色号。纯前端，开箱即用。
...
| 模式 | 操作 | 适用场景 |
|------|------|---------|
| **点击取色** | 点击图片任意位置（支持采样网格 3×3 / 5×5 / 7×7） | 精确取色 |
| **吸管取色** 👑 | 鼠标悬停即自动取色 | 快速浏览多个颜色 |
| **区域取色** | 拖拽框选区域，自动计算平均色 | 渐变色、大面积区域 |
| **手动输入 HEX** | 点击 HEX 值行，直接输入 `#RRGGBB` 回车确认 | 精准输入已知色号 |
...
- **HSL + A 滑块微调**：取色后拖动 H/S/L/A 滑块微调，色环实时联动
- **颜色名称识别**：自动匹配 150+ 常见颜色名称（珊瑚红 Coral 等）
- **WCAG 对比度检查**：输入另一颜色 HEX，自动计算 AA/AAA 评级

### 🎨 色彩搭配

- **色盲模拟器**：取色后显示红/绿/蓝色盲三种模拟预览，点击色块复制 HEX
- **图像滤镜**：亮度/对比度/饱和度三滑块，实时影响取色结果
- **色彩搭配**：互补色、近似色、三角调和三组配色方案，每行一键复制全部色号
...
- **取色记录**：最多保存 20 条，悬浮显示色号、取色时间和坐标
- **星标收藏**：标记重要颜色，收藏项自动置顶，支持筛选仅看收藏
- **清空全部**：仅清除非收藏记录；一键复制全部 HEX 到剪贴板
- **搜索过滤**：支持按 HEX / RGB / HSL 格式搜索
- **数据持久化**：刷新页面不丢失
- **撤销取色**：`Ctrl+Z` 撤销最近一次取色

### 📤 导出与分享

| 功能 | 说明 |
|------|------|
| **📦 导出代码** | CSS 变量 / Tailwind v4 / SCSS / JSON / CSV |
